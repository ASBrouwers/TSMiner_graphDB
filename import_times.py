...
def create_plots(df):
	create_plot(df, "r == 8000", 'k', 'time', 'k', 'Time (s)', "", "k")
	...
	combined_plot(df, "r == 8000", 'k', 'time', 'k', 'Time (s)', "", "k")
	combined_plot(df, "k == 3", 'r', 'time', 'Dataset size (cases)', 'Time (s)', "", 3)
...
def combined_plot(df, q, x, y, x_label, y_label, label, k):
	ax = df.query("abst == 'list' and " + q).plot(x, y, label=f"List-{k}-past", marker='o', figsize=(5, 4))
	df.query("abst == 'set' and " + q).plot(x, y, label=f"Set-{k}-past", ax=ax, marker='o')
	df.query("abst == 'list_prom' and " + q).plot(x, y, label=f"List-{k}-past (ProM)", ax=ax, marker='o')
	df.query("abst == 'set_prom' and " + q).plot(x, y, label=f"Set-{k}-past (ProM)", ax=ax, marker='o')
...
def create_tables(df):
	...
	print(df.query("abst == 'list' and r == 8000").sort_values(by='k').to_latex(index=False, escape=False,
																			 header=["Abstraction", "$k$", "Cases",
																					 "Time (s)"], float_format="%.2f"))
	print(df.query("abst == 'set' and r == 8000").sort_values(by='k').to_latex(index=False, escape=False,
																				 header=["Abstraction", "$k$", "Cases",
																						 "Time (s)"],
																				 float_format="%.2f"))
...
def add_prom(df):
	...
	df.loc[len(df.index)] = ['list_prom', 15, 8000, 315]
	...
	df.loc[len(df.index)] = ['set_prom', 15, 8000, 116]
